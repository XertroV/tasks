...
def test_schema_text_is_file_kind_level_not_per_file(runner, tmp_path, monkeypatch):
    ...
    assert ".backlog/index.yaml" in result.output
    assert ".backlog/.context.yaml" in result.output
    assert ".backlog/.sessions.yaml" in result.output
    assert ".backlog/config.yaml" in result.output
...
def test_schema_compact_output(runner, tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0
    assert ".backlog File-Type Schema (Compact)" in result.output
    assert "File Kinds (compact)" in result.output
    assert "Root index | .backlog/index.yaml | yaml" in result.output
